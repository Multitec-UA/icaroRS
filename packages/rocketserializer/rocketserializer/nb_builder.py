...
class NotebookBuilder:
    ...
    def build_imports(self, nb: nbf.v4.new_notebook) -> nbf.v4.new_notebook:
        ...
        text = (
            ...
            + "TrapezoidalFins, EllipticalFins, FreeFormFins, RailButtons, NoseCone, "
            + "Tail, Parachute\n"
        )
    ...
    def build_fins(self, nb: nbf.v4.new_notebook) -> nbf.v4.new_notebook:
        ...
        if len(self.parameters["elliptical_fins"]) > 0:
            ...
            pass
        # free form fins
        if len(self.parameters["freeform_fins"]) > 0:
            self.freeform_fins_check = True
            fin_counter += len(self.parameters["freeform_fins"])
            text = "freeform_fins = {}\n"
            nb["cells"].append(nbf.v4.new_code_cell(text))
            for i in range(len(self.parameters["freeform_fins"])):
                ff = self.parameters["freeform_fins"][str(i)]
                text = f"freeform_fins[{i}] = FreeFormFins(\n"
                text += f"    n={ff['number']},\n"
                text += f"    shape_points={ff['shape_points']},\n"
                text += f"    rocket_radius={self.parameters['rocket']['radius']},\n"
                text += f"    cant_angle={ff['cant_angle']},\n"
                text += f"    name='{ff['name']}',\n"
                text += ")\n\n"
                nb["cells"].append(nbf.v4.new_code_cell(text))
            logger.info("[NOTEBOOK BUILDER] Free-form fins created.")
