...
def search_free_form_fins(bs, elements):
    """Search for free-form fins in the bs and return the settings as a dict.
    It is flexible in the sense that it can handle multiple free-form fin sets.
    ...
    -------
    settings : dict
        Dictionary with the settings for the free-form fins. The keys are
        integers and the values are dicts containing the settings for each
        free-form fin set. The keys of the free-form fin set dicts are:
        "name", "number", "cant_angle", "position", "section", "shape_points".
        shape_points is a list of [x, y] lists passed through unchanged from the
        OpenRocket finpoints: OpenRocket already stores them with +x toward the
        tail and (0, 0) at the root leading edge, matching RocketPy's
        add_free_form_fins convention (no transform needed).
    """
    settings = {}
    fins = bs.findAll("freeformfinset")
    logger.info("A total of %d free-form fin sets were detected", len(fins))

    if len(fins) == 0:
        logger.info(
            "No free-form fins were detected, returning empty dictionary"
        )
        return settings

    for idx, fin in enumerate(fins):
        logger.info(
            "Starting collecting the settings for the free-form fin set number '%d'",
            idx,
        )
        label = fin.find("name").text
        try:
        ...
        section = (
            fin.find("crosssection").text if fin.find("crosssection") else None
        )
        logger.info("Crosssection format retrieved")

        points = fin.find("finpoints").findAll("point")
        shape_points = [[float(pt["x"]), float(pt["y"])] for pt in points]
        logger.info("Retrieved %d shape points", len(shape_points))
        ...
        logger.info(
            "Free-form fin set number '%d' was defined:\n%s",
            idx,
            _dict_to_string(fin_settings, indent=23),
        )

    logger.info("Finished collecting all the free-form fins.")
    return settings
